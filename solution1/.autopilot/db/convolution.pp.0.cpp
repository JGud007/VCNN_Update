...
float conv_bias[20] = {-0.080922, -0.054016, 0.010864, -0.078121, -0.011839, 0.072157, -0.137894, -0.161596, -0.032325, -0.01468, -0.258716, -0.154862, 0.140349, -0.3088, -0.065761, 0.025168, -0.126911, 0.24899, -0.044372, 0.133308};

inline float conv3d1(Layer current, int fn, int ksize, int channels, int h, int w){
 float r = 0;
 ...
 for (int fn = 0; fn < fnum; fn++){
  for(int h = offset_idx, hc = 0; h < height; h+=stride, hc++)
   for(int w = offset_idx, wc = 0; w < width; w+=stride, wc++){
    ...
    float temp = conv3d1(current, fn*filterSize, ksize, channels, h, w);
